...
class Retriever:
    ...
    def build_faiss_index(self, dataset_name, batch_size=32, num_chunks=None):
        index_name = self.config["embedding_model"].split("/")[-1]
    ...
    def query_faiss_index(self, dataset_name, qa_dataset_name, queries, knn=8):
        index_name = self.config["embedding_model"].split('/')[-1]
        dataset_dir = os.path.join(CORPUS_DIR, f"{dataset_name}")
        ...
        query_embeddings = self.emb_model.encode(queries)  # Already batches internally
    ...
    @classmethod
    def index_exists(cls, corpus_name, index_name):
        config = yaml.safe_load(open(FAISS_DEFAULT_CONFIG, "r"))
        dataset_dir = os.path.join(CORPUS_DIR, corpus_name, "index", config["embedding_model"])
        print(f"searching for index at {dataset_dir}")
        index_path = os.path.join(dataset_dir, f"{index_name}.index")
        metadata_path = os.path.join(dataset_dir, "metadatas.jsonl")
